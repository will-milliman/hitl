 ...
 */
export function notifyPrReviewNeeded(itemType: 'task', itemId: number, itemTitle: string, commentCount: number): void {
  show(`PR Review Comments (Task #${itemId})`, `${commentCount} unresolved comment(s) on "${itemTitle}"`, 'prReviewNeeded');
}
...
export function notifyTaskCompleted(taskId: number, taskTitle: string): void {
  show('Task Completed', `Task #${taskId}: ${taskTitle}`, 'taskCompleted');
}
...
export function notifyCronError(stepName: string, errorMessage: string): void {
  show(`Cron Error: ${stepName}`, errorMessage.slice(0, 200), 'cronErrors');
}
