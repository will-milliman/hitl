 ...
 */
export async function getPullRequest(cwd: string, prNumber: number): Promise<PullRequest> {
  return ghJson<PullRequest>(
    [
      ...
      String(prNumber),
      '--json',
      'number,url,state,isDraft,title,headRefName,baseRefName,author,reviewDecision,statusCheckRollup',
    ],
    cwd,
  );
}
 ...
 */
export async function getPullRequestByUrl(prUrl: string, cwd: string): Promise<PullRequest> {
  return ghJson<PullRequest>(
    [
      'pr',
      'view',
 ...
 */
export async function findPullRequest(cwd: string, head: string, base: string): Promise<PullRequest | null> {
  try {
    const prs = await ghJson<PullRequest[]>(
      [
        ...
        '1',
        '--json',
        'number,url,state,isDraft,title,headRefName,baseRefName,author',
      ],
      cwd,
    );
 ...
 */
export async function isPrMerged(cwd: string, prNumber: number): Promise<boolean> {
  try {
 ...
 */
export async function getPrReviewComments(cwd: string, owner: string, repo: string, prNumber: number): Promise<ReviewComment[]> {
  try {
    return await ghJson<ReviewComment[]>(
      ['api', `repos/${owner}/${repo}/pulls/${prNumber}/comments`, '--paginate', '-q', '.'],
      cwd,
    );
  } catch (err) {
 ...
 */
export async function getPrIssueComments(cwd: string, owner: string, repo: string, prNumber: number): Promise<ReviewComment[]> {
  try {
    return await ghJson<ReviewComment[]>(
      ['api', `repos/${owner}/${repo}/issues/${prNumber}/comments`, '--paginate', '-q', '.'],
      cwd,
    );
  } catch (err) {
 ...
 */
export function findUnresolvedThreads(comments: ReviewComment[], prAuthorLogin: string): ReviewComment[] {
...
export function formatCommentsForPrompt(comments: ReviewComment[]): string {
  ...
  const lines = [`There are ${comments.length} unresolved review comment(s) on your pull request that need to be addressed:\n`];
  ...
  lines.push(`Please address each comment by making the necessary code changes. After making changes, commit and push them.`);
