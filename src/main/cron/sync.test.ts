...
import { makeWorkItem, makeTask } from '../test-utils/factories'
...
describe('syncWorkItems', () => {
  ...
  it('creates new tasks in PROFILE_ASSIGNMENT state', async () => {
    ...
    vi.mocked(getWorkItems)
      ...
      .mockResolvedValueOnce([
        makeWorkItem({ id: 1001, title: 'New task', state: 'Active', parentId: 2001 }),
      ])
      // Second call: parent story details
      .mockResolvedValueOnce([
        makeWorkItem({ id: 2001, title: 'Parent story', type: 'User Story' }),
      ])
  ...
  it('creates blocked tasks in BLOCKED state', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([
      makeWorkItem({ id: 1001, title: 'Blocked task', state: 'Blocked' }),
    ])
  ...
  it('transitions existing task to BLOCKED when Azure state is Blocked', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([
      makeWorkItem({ id: 1001, title: 'Task', state: 'Blocked' }),
    ])

    // Task exists in PROFILE_ASSIGNMENT
    mockDb.task.findUnique.mockResolvedValueOnce(
      makeTask({ id: 1001, state: GridState.PROFILE_ASSIGNMENT })
    )
  ...
  it('transitions BLOCKED task back to PROFILE_ASSIGNMENT when unblocked', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([
      makeWorkItem({ id: 1001, title: 'Task', state: 'Active' }),
    ])

    // Task exists in BLOCKED state
    mockDb.task.findUnique.mockResolvedValueOnce(
      makeTask({ id: 1001, state: GridState.BLOCKED })
    )
  ...
  it('updates title/azureUrl without changing state for existing non-blocked tasks', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([
      makeWorkItem({ id: 1001, title: 'Updated title', state: 'Active' }),
    ])

    // Task exists in PR_REVIEW state
    mockDb.task.findUnique.mockResolvedValueOnce(
      makeTask({ id: 1001, state: GridState.PR_REVIEW, title: 'Old title' })
    )
  ...
  it('handles tasks without a parent story', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([
      makeWorkItem({ id: 1001, title: 'Orphan task', state: 'Active' }),
    ])
  ...
  it('processes multiple tasks in a single sync', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([
      ...
      makeWorkItem({ id: 2001, title: 'Shared story', type: 'User Story' }),
    ])
