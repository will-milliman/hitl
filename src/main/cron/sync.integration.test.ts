...
describe('syncWorkItems (integration)', () => {
  it('creates a new task and parent story in the real DB', async () => {
    ...
    vi.mocked(getWorkItems)
      .mockResolvedValueOnce([makeWorkItem({ id: 5001, title: 'Implement login', state: 'Active', parentId: 6001 })])
      .mockResolvedValueOnce([makeWorkItem({ id: 6001, title: 'Authentication feature', type: 'User Story' })]);
  ...
  it('creates a blocked task when Azure state is Blocked', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([makeWorkItem({ id: 5002, title: 'Blocked task', state: 'Blocked' })]);
  ...
  it('transitions existing task to BLOCKED and back to PROFILE_ASSIGNMENT', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([makeWorkItem({ id: 5003, title: 'Toggle task', state: 'Blocked' })]);
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([makeWorkItem({ id: 5003, title: 'Toggle task', state: 'Active' })]);
  ...
  it('updates title without changing state for existing tasks', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([makeWorkItem({ id: 5004, title: 'Updated title', state: 'Active' })]);
  ...
  it('handles multiple tasks with a shared parent story', async () => {
    ...
    vi.mocked(getWorkItems)
      ...
      .mockResolvedValueOnce([makeWorkItem({ id: 6010, title: 'Epic story', type: 'User Story' })]);
  ...
  it('preserves storyId when parent is not in current WIQL result', async () => {
    ...
    vi.mocked(getWorkItems).mockResolvedValueOnce([makeWorkItem({ id: 5020, title: 'Existing task updated', state: 'Active' })]);
