...
import { ExternalLink, ActionLink, CheckboxCell, Placeholder, ErrorIndicator } from '../components/common'
...
interface TaskPRReviewGridProps {
  tasks: Task[]
}

export function TaskPRReviewGrid({ tasks }: TaskPRReviewGridProps) {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('storyId', {
        ...
        cell: (info) => (
          <ExternalLink
            href={`https://dev.azure.com/org/project/_workitems/edit/${info.getValue()}`}
          >
            {info.getValue()}
          </ExternalLink>
        ),
      ...
      columnHelper.accessor('prMerged', {
        ...
        cell: (info) => (
          <CheckboxCell checked={info.getValue()} disabled />
        ),
      }),
